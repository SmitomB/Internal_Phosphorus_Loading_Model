...
# Motivation
The codes in this R notebook are used to simulate the P dynamics in the study period (1983-2018).
...
# Packages
Let us load the packages needed to run the code.
...
# To print all the columns of the tibble
...
Let us load the data stored in different repositories to run the model.
...
# the t column is reassigned to have continuously increasing natural numbers
RoutMOdin$t = c(1:tmax)


# transformation parameters here for box-cox transformation
l1 = 0
l2 = 0

# custom functions loaded here
suppressMessages(source("Pmodels/JLPMfunv16p001.R"))  

# Fix watersheds concentrations using Dn's spreadsheet
...
# Adding the correct inflow P concentration
...
# Initial P in water and sediments

In the following code, we assign the initial values of P mass in the water-column and sediment layer. 
...
We now setup the model in a format so we can run the ODIN package. 
...
Let us set up test parameters to check whether the model is working.
...
mod_wrap_v0 <- function(inp_dat=inp_lis,
  ...
  # Adding concentration to the data frame
  ...
  # creating a data frame that contains only the concentration in the 4 segments
  y_c <- y_c[,c("yrmo","C1","C2","C3","C4")]
  
  # Using melt function to convert the columns into rows, taking yrmo as the id.vars
...
RAM <- MCMC(p     = logpost, #objective function
            ...
            n     = n.iter, # iterations
            adapt = T, # adaptive sampling is used 
...
The data visualization of the model results are carried out in subsequent parts.
