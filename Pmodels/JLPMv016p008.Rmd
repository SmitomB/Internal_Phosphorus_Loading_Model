...
subtitle: "Part8: Management scenarios - 1"
author: "Smitom Borah"
date: "04/29/2023"
output: 
  html_notebook:
...
# Motivation
In this notebook, we visualize the future trends of TP in water and sediment columns under different management scenarios under RCP 8.5. 
...
# postPsmpl <- RAM$samples[seq((nrow(RAM$samples)-20000),nrow(RAM$samples),length.out =N),]
...
# loop.seed <- sample(seq(1000,5000, by = 1 ), N)
for (i in 1:N){
...
## TP in segment 1
```{r}
ggplot(data = CI_TP1_H00)+
  geom_line(aes(x = cale_date, y = `50%`, color = "Median TP"))+
  geom_ribbon(aes(x = cale_date, ymin = `pred5%`, ymax = `pred95%`, fill = "90% PI"), alpha = 0.2)+ 
  # geom_point(aes(x = cale_date, y = TPugL, color = "Observed TP"), alpha = 0.4)+
  theme_bw()+xlab("")+
  coord_cartesian(ylim=c(0, 700))+scale_y_continuous(expand = c(0, 0))+
  theme(legend.position = c(0.70, 0.80))+
  geom_vline(xintercept = ymd('2018-12-01'),linetype ="dashed",size=.6)+
  scale_fill_manual("",values=c("grey12"))+
  scale_x_date(breaks = as.Date(c("1983-01-01", "2000-01-01", "2018-01-01", "2035-01-01", "2055-01-01", "2075-01-01", "2090-01-01")),date_labels = "%Y")+
  scale_colour_manual(NULL,
                      values=c(`Median TP` = "steelblue"),
  ...
  labs(title = "Segment 1")
```

## Sediment P concentration
```{r}
ggplot(data = sedP_H00)+
  geom_line(aes(x = cale_date, y = `50%`, color = "Median sediment P"))+
  geom_ribbon(aes(x = cale_date, ymin = `pred5%`, ymax = `pred95%`, fill = "90% PI"), alpha = 0.2)+
  theme_bw()+xlab("")+
  coord_cartesian(ylim=c(0, 300))+scale_y_continuous(expand = c(0, 0))+
  geom_vline(xintercept = ymd('2018-12-15'),linetype ="dashed",size=.6)+
  scale_fill_manual("",values=c("grey12"))+
  scale_x_date(breaks = as.Date(c("1983-01-01", "2000-01-01", "2018-01-01", "2035-01-01", "2055-01-01", "2075-01-01", "2090-01-01")),date_labels = "%Y")+
  scale_colour_manual(NULL,
                      values=c(`Median sediment P` = "steelblue"),
                      guide = guide_legend(override.aes = list(
                        linetype = c("solid"),
                        shape = c(NA)))) + 
...
# plots
P_moPlot <- ggplot()+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H00_50, color = "H00"))+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H050_50, color = "H050"))+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H050d_50, color = "H050d"))+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H500_50, color = "H500"))+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H5050_50, color = "H5050"))+
  geom_line(data = P_w.modf, aes(x = cale_date,
                                 y = H5050d_50, color = "H5050d"))
P_moPlot
```

## Annual values
```{r echo=TRUE, message=FALSE, warning=FALSE}
P_w.anndf <- P_w.modf %>% mutate(year = year(cale_date)) %>% group_by(year) %>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))

## Mitigation plan MP
P_w.annPlot <- ggplot()+
  geom_line(data = P_w.anndf, aes(x = year, y = H00_50, color = "50% ext P \n reduction"), size = 0.5)+
  geom_line(data = P_w.anndf, aes(x = year, y = H050_50, color = "Approach I: \n 50% int P reduction"), size = 0.5)+
  geom_line(data = P_w.anndf, aes(x = year, y = H5050_50, color = "Approach I: \n 50% (ext P + int P) reduction"), size = 0.5)+
  geom_line(data = P_w.anndf, aes(x = year, y = H500_50, color = "Approach II: \n 50% int P reduction"), size = 0.5)+
  geom_line(data = P_w.anndf, aes(x = year, y = H050d_50, color = "Approach II: \n 50% (ext P + int P) reduction"), size = 0.5)+
  geom_line(data = P_w.anndf, aes(x = year, y = H5050d_50, color = "No reduction"), size = 0.5)+
  ...
  scale_colour_manual(NULL,
                      values=c(`Approach I: \n 50% int P reduction` = "blue",
                               `Approach I: \n 50% (ext P + int P) reduction` = "red",
                               `Approach II: \n 50% int P reduction` = "green",
                               `Approach II: \n 50% (ext P + int P) reduction` = "purple",
                               `50% ext P \n reduction` = "brown",
                               `No reduction` = "black"),
                      guide = guide_legend(override.aes = list(
                        ...
                        ncol = 1,
                        nrow = 6)) + 
  ylab(expression(Average~total~phosphorus~"("*mu*g/L*")"))+
  xlab("Year")+
...
```{r}
# Water-column P plot
P_w.annPlot1 <- ggplot(data = P_w.anndf)+
  geom_line(aes(x = year, y = H500_50, color = "H500"), size = 1, lty = 2)+
  geom_line(aes(x = year, y = H050_50, color = "H050"), size = 1, lty = 3)+
  geom_line(aes(x = year, y = H5050_50, color = "H5050"), size = 1, lty = 5)+
  geom_line(aes(x = year, y = H050d_50, color = "H050d"), size = 1, lty = 4)+
  geom_line(aes(x = year, y = H5050d_50, color = "H5050d"), size = 1, lty = 6)+
  geom_line(aes(x = year, y = H00_50, color = "H00"), size = 1, lty = 1)+
  scale_colour_manual(NULL,
                      ...
                      labels = c("1: No reduction",
                                 ...
                                 "6: -50% (IPL + EPL) (D)"),
                      guide = guide_legend(override.aes = list(
                        linetype = c(1,2,3,4,5,6),
                        color = c("black", "blue", "brown","lime green", "red", "orange"),
                        shape = c(NA, NA, NA,NA, NA, NA)),
                        size = c(2,2,2,2,2,2),
                        nrow = 3,
                        ncol = 3))+
  ...
  scale_x_continuous(breaks = c( 1983,2000, 2018, 2050, 2090),expand = c(0, 0))

P_w.annPlot1


# Colored plot
# png(file="OutputFiles/JLPMv016_TPManProj_plot_colored.png",width = 6, height = 3, units = 'in', res = 1200)
# P_w.annPlot1
# dev.off()
```

```{r}
P_s.anndf <- P_s.modf %>% mutate(year = year(cale_date)) %>% group_by(year) %>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))


# Sediment P plot
P_s.annPlot1 <- ggplot(data = P_s.anndf)+
  geom_line(aes(x = year, y = H500_50, color = "H500"), size = 1, lty = 2)+
  geom_line(aes(x = year, y = H050_50, color = "H050"), size = 1, lty = 3)+
  geom_line(aes(x = year, y = H5050_50, color = "H5050"), size = 1, lty = 5)+
  geom_line(aes(x = year, y = H050d_50, color = "H050d"), size = 1, lty = 4)+
  ...
  theme(
    panel.grid.major = element_blank(),
        ...
        axis.title = element_text(size = 10, color = "black"),
        axis.ticks = element_line(color = "black"))+
  scale_y_continuous(limits = c(0,100),breaks = c(0,25,50,75),expand = c(0, 0))+
  scale_x_continuous(breaks = c( 1983,2000, 2018, 2050, 2090),expand = c(0, 0))

P_s.annPlot1

# Colored plot
# png(file="OutputFiles/JLPMv016_SedPManProj_plot_colored.png",width = 6, height = 3, units = 'in', res = 1200)
# P_s.annPlot1
# dev.off()
...
# Saving the plot
ggsave("OutputFiles/JLPMv016_PwPs_plot.png",Pw_Ps_plot,
       height=7,width=7,units='in',dpi=1200)
dev.off()
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Annual values between study period: 2009-2018 (table 1)
P_w.anndf.comp1 <- P_w.anndf[which(P_w.anndf$year %in% c((seq(2009,2018)))),] %>% mutate(Period = rep("Study", 10))
P_w.anndf.comp1

# Annual values between projection period: 2081-2090 (table 2)
P_w.anndf.comp2 <- P_w.anndf[which(P_w.anndf$year %in% c((seq(2081,2090)))),] %>% mutate(Period = rep("Project", 10))
P_w.anndf.comp2

# Annual values between projection period: 2019-2028 (table 3)
P_w.anndf.comp3 <- P_w.anndf[which(P_w.anndf$year %in% c((seq(2019,2028)))),] %>% mutate(Period = rep("Project", 10))
P_w.anndf.comp3

# decadal average
## Study period: 2009-2018 (table 4)
P_w.annvec10.comp1 <- P_w.anndf.comp1 %>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_w.annvec10.comp1

## Projection period, last decade: 2081-2090 (table 5)
P_w.annvec10.comp2 <- P_w.anndf.comp2%>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_w.annvec10.comp2

## Projection period, first decade: 2019-2028 (table 6)
P_w.annvec10.comp3 <- P_w.anndf.comp3%>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_w.annvec10.comp3

# Percentage reductions
## last decades of study and projection period (table 7)
P_w.annvec10.comp12 <- ((P_w.annvec10.comp1-P_w.annvec10.comp2)/(P_w.annvec10.comp1))*100
P_w.annvec10.comp12

## last decade of study period and first 
## decade of projection period (table 8)
P_w.annvec10.comp13 <- ((P_w.annvec10.comp1-P_w.annvec10.comp3)/(P_w.annvec10.comp1))*100
P_w.annvec10.comp13
...
```{r}
# Annual values between study period: 2009-2018 (table 1)
P_s.anndf.comp1 <- P_s.anndf[which(P_s.anndf$year %in% c((seq(2009,2018)))),] %>% mutate(Period = rep("Study", 10))
P_s.anndf.comp1

# Annual values between projection period: 2081-2090 (table 2)
P_s.anndf.comp2 <- P_s.anndf[which(P_s.anndf$year %in% c((seq(2081,2090)))),] %>% mutate(Period = rep("Project", 10))
P_s.anndf.comp2

# Annual values between projection period: 2019-2028 (table 3)
P_s.anndf.comp3 <- P_s.anndf[which(P_s.anndf$year %in% c((seq(2019,2028)))),] %>% mutate(Period = rep("Project", 10))
P_s.anndf.comp3

# decadal average
## Study period: 2009-2018 (table 4)
P_s.annvec10.comp1 <- P_s.anndf.comp1 %>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_s.annvec10.comp1

## Projection period: 2081-2090 (table 5)
P_s.annvec10.comp2 <- P_s.anndf.comp2%>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_s.annvec10.comp2

## Projection period: 2019-2028 (table 6)
P_s.annvec10.comp3 <- P_s.anndf.comp3%>% summarise_at(vars(H00_50, H050_50, H050d_50, H500_50, H5050_50, H5050d_50), funs(mean))
P_s.annvec10.comp3

# Percentage reductions
## last decades of study and projection period (table 7)
P_s.annvec10.comp12 <- ((P_s.annvec10.comp1-P_s.annvec10.comp2)/(P_s.annvec10.comp1))*100
P_s.annvec10.comp12

## last decade of study period and first 
## decade of projection period (table 8)
P_s.annvec10.comp13 <- ((P_s.annvec10.comp1-P_s.annvec10.comp3)/(P_s.annvec10.comp1))*100
P_s.annvec10.comp13
```
## Percentage reduction between 2018 and 2090 annual values
```{r}
# 50% external load reduction
## 2018
T_w.H500.2018 <- P_w.anndf[which(P_w.anndf$year == 2018),"H500_50"] # 70.10942
T_w.H500.2018

## 2090
T_w.H500.2090 <- P_w.anndf[which(P_w.anndf$year == 2090),"H500_50"] # 42.81334
T_w.H500.2090

# % reduction
((T_w.H500.2018 - T_w.H500.2090 )/T_w.H500.2018)*100 # 38.9
```

## Percentage reduction between 2019 and 2090 annual values
```{r}
# 50% external load reduction
## 2019
T_w.H500.2019 <- P_w.anndf[which(P_w.anndf$year == 2019),"H500_50"] # 70.10942
T_w.H500.2019
...
# % reduction
((T_w.H500.2019 - T_w.H500.2090 )/T_w.H500.2019)*100 # 38.9
```
