...
# Motivation
In this notebook, we developed a random forest model that can be used to predict bottom to surface P ratio that is used to calculate the surface P concentration in the Jordan lake model. Most of the codes in this notebook is similar to the codes written in the notebook, *Additional025_Stratification_v02082022.Rmd*.
...
2. 5weeks M.A. air temperature ($^\circ C$)
3. 2weeks M.A. air temperature ($^\circ C$)
...
# Loading functions script created by Dario
...
# (as substitute for surface water temperature)
...
# (as substitute for bottom water temperature)
...
CollectData <- function(FileName = "Segment2_Pdata.xlsx", SheetName = "report"){
  ...
  ## Option 1: Selecting only depth, monitoring site and P concentration from Seg_P
  # Seg_Pdat <- Seg_P[,c("Date","Site","P_location", "P_conc", "Depth" )] %>% drop_na()
  
  ## Option 2: Selecting only depth and P concentration from Seg_P
  ...
  #             values_from = c("P_conc", "Depth")) %>% drop_na()
...
Although these files were originally saved from Additional025_Stratification_v02082022_RF4.Rmd, we copy the related codes into this notebook as well.
