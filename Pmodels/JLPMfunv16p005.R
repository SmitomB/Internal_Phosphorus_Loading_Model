...
lakemod_v0 <- odin::odin({
  ...
  deriv(M1) <- (S1*Rs)*(ThetaR^(Tmp-20)) +(Q1in*C1in)*(1-Sh) +(Q21*M2/V2)              -((M1*(Ks+Vs*A1/V1))*(ThetaV^(Tmp-20)) +(Q12*M1/V1))                - (M1*Q1h/V1)                          + ((Q1inh*C1in)*(1-Sh))
  deriv(M2) <- (S2*Rs)*(ThetaR^(Tmp-20)) +(Q2in*C2in)*(1-Sh) +(Q32*M3/V3) +(Q12*M1/V1) -((M2*(Ks+Vs*A2/V2))*(ThetaV^(Tmp-20)) +(Q23*M2/V2)+(Q21*M2/V2))    - (M2*Q2h/V2)                           + ((Q2inh*C2in)*(1-Sh))
  deriv(M3) <- (S3*Rs)*(ThetaR^(Tmp-20)) +(Q3in*C3in)*(1-Sh) +(Q43*M4/V4) +(Q23*M2/V2) -((M3*(Ks+Vs*A3/V3))*(ThetaV^(Tmp-20)) +(Q34*M3/V3)+(Q32*M3/V3))    - (M3*Q3h/V3)                          + ((Q3inh*C3in)*(1-Sh))
  deriv(M4) <- (S4*Rs)*(ThetaR^(Tmp-20)) +(Q4in*C4in)*(1-Sh) +            (Q34*M3/V3)  -((M4*(Ks+Vs*A4/V4))*(ThetaV^(Tmp-20)) +(M4*Q4out/V4)+(Q43*M4/V4))  - (M4*Q4h/V4)                          + ((Q4inh*C4in)*(1-Sh))
  ...
  output(V3) <- V3
  output(V4) <- V4
  output(inflow1) <- (Q1in*C1in)*(1-Sh)
  output(recycle) <- (S1*Rs)*(ThetaR^(Tmp-20))
  output(settling) <- (M1*(Ks+Vs*A1/V1))*(ThetaV^(Tmp-20))
