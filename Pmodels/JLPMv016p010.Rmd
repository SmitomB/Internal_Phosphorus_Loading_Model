...
Time required to run the entire notebook: ~ ???


# Motivation
In this notebook, we develop future trends of TP in water and sediment columns under the scenario if historical temperatures persisted in the future. The final visualization are done in subsequent parts.
...
for (i in 1:nhist){
  # i <- 1 #TODO remove this line in the final cleaned version
...
PI_func <- function(PI, Cx, nhistx = nhist, nparx = npar, rnumx = rnum){
  ...
  for (i in 1:nhistx){
    ...
    for (j in 1:nparx){
      ...
      Cxh.lst[[i]] <- tmpx.df %>% 
        # mutate(central = apply(select(.,-date),1, median)) # Central value is median
...
plot_basics+
  geom_line(data = P1concL00.df, aes(x = date, y = `hyd50%`, color = "Median TP"))+
  geom_ribbon(data = P1concL00.df, aes(x = date, ymin = `hyd5%`, ymax = `hyd95%`), fill = "black", alpha = 0.3)+
  geom_ribbon(data = P1concL00.df, aes(x = date, ymin = `parhyd5%`, ymax = `parhyd95%`), fill = "green", alpha = 0.3)+ 
  geom_ribbon(data = P1concL00.df, aes(x = date, ymin = `all5%`, ymax = `all95%`), fill = "red", alpha = 0.3)+
...
# Plotting the results
plot_basics+
  geom_line(data = SconcL00.df, aes(x = date, y = `hyd50%`, color = "Median TP"))+
  geom_line(data = SconcL00.df, aes(x = date, y = `hyd_mean`, color = "Mean TP"))+
...
# Plotting the results
plot_basics+
  geom_line(data = SresL00.df, aes(x = date, y = `hyd50%`, color = "Median TP"))+
...
# Saving the file (last saved on 06/08/2023 10:00 PM) with mean central value
